...
func _physics_process(delta: float):
	...
	if DisplayServer.is_touchscreen_available() or Input.is_action_pressed("move_up"):
		...
		if not DisplayServer.is_touchscreen_available():
			...
			engine_force *= Input.get_action_strength("move_up")
	else:
		engine_force = 0.0
	...
	if Input.is_action_pressed("move_down"):
		...
		engine_force *= Input.get_action_strength("move_down")
